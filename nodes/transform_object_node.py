...
class TransformObjectNodeCnt(ConstantNodeCnt):
    ...
    def init(self, context):
        ...
        x = self.inputs.new(CntSocketTypes.Float, "scale X")
        y = self.inputs.new(CntSocketTypes.Float, "scale Y")
        z = self.inputs.new(CntSocketTypes.Float, "scale Z")
    ...
    def camera_update(self):
        if self.inputs[0].input_value:
            ...
            obj.rotation_euler = (math.radians(self.inputs[4].input_value), math.radians(self.inputs[5].input_value),
                                  math.radians(self.inputs[6].input_value))
